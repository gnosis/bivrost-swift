        ...
        describe("ParameterParser") {
            ...
            context("number types") {
                ...
                it("should throw when parsing non-length types with length") {
                    ...
                    expect { try ParameterParser.parseParameterType(from: ["type": "function16"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                ...
                it("should throw when parsing length types with invalid length") {
                    ...
                    expect { try ParameterParser.parseParameterType(from: ["type": "uint9"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                    expect { try ParameterParser.parseParameterType(from: ["type": "uint7"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                    expect { try ParameterParser.parseParameterType(from: ["type": "int257"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                    expect { try ParameterParser.parseParameterType(from: ["type": "int9"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                    expect { try ParameterParser.parseParameterType(from: ["type": "int7"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                    expect { try ParameterParser.parseParameterType(from: ["type": "int2000"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                    expect { try ParameterParser.parseParameterType(from: ["type": "uint2000"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                    expect { try ParameterParser.parseParameterType(from: ["type": "bytes64"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                    expect { try ParameterParser.parseParameterType(from: ["type": "bytes0"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                    expect { try ParameterParser.parseParameterType(from: ["type": "bytes000"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                    expect { try ParameterParser.parseParameterType(from: ["type": "uint17[]"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                    expect { try ParameterParser.parseParameterType(from: ["type": "int33[16][16]"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                    expect { try ParameterParser.parseParameterType(from: ["type": "uint0016"]) }.to(throwError(ParsingError.parameterTypeInvalid))
                }
            }
        }
    }
